...
const FilterModal = props => {
  ...
  const handleChangeOrg = org => {
    ...
    const newFilter = {};
    if (org.length)
      newFilter['organizationUnit.organizationUnitId'] = org[0]._id;
  ...
  return (
    <Modal isVisible={isVisible} style={{height: 'auto', borderRadius: 10}}>
      <View style={{backgroundColor: '#fff', borderRadius: 10}}>
              ...
              <View onPress={() => setShowDatePicker(true)}>
                <View style={{alignItems: 'flex-end', ...styleRight}}>
                  <Text>
                    {`${
                      start
                        ? moment(start, DATE).format(DATE_FORMAT)
                        : moment().startOf('weeks').format(DATE_FORMAT)
                    } - ${
                      end
                        ? moment(end, DATE).format(DATE_FORMAT)
                        : moment().endOf('weeks').format(DATE_FORMAT)
                    } `}
                    <Icon
                      type="FontAwesome"
                      name="caret-down"
          ...
          {enableFilterOrg && (
            <View>
              <View
                ...
                itemDivider
                style={{
                  borderTopLeftRadius: enableDatePicker ? 0 : 10,
                  borderTopRightRadius: enableDatePicker ? 0 : 10,
                  ...styles,
                }}>
        ...
        <View padder style={{flexDirection: 'row', marginBottom: 10}}>
          <TouchableOpacity
            block
            onPress={handleSave}
            style={{
            ...
            <Icon name="check" type="Feather"  style={{textAlign: 'center', color: 'white'}}/>
          </TouchableOpacity>
          <TouchableOpacity
            block
            onPress={onClose}
            ...
            warning>
            <Icon name="close" type="AntDesign" style={{textAlign: 'center', color: 'white'}} />
          </TouchableOpacity>
        </View>
      </View>
