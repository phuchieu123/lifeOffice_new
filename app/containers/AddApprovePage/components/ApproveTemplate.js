...
function ApproveTemplate(props) {
    ...
    const onSendApprove = () => {
        ...
        const res = onAddApprove(body)
        if (res) {
            DeviceEventEmitter.emit('ApproveProject', { res: res })
        }
        return res
    }
        ...
        <Container>
            <BackHeader
                ...
                rightHeader={
                    isLoading
                        ? <Spinner />
                        : <Icon active name="send" onPress={() => { onSendApprove; navigation.navigate('Approve') }} style={{ color: '#fff' }} />
                }
            />
...
export default compose(withConnect)(ApproveTemplate);
