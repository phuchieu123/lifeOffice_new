...
export function* createElectricIndicator(action) {
  let { data } = action;
  ...
  try {
    ...
    if (response.success) {
      yield put(actions.createElectricIndicatorSuccess(response.data));
    } else {
      yield put(actions.createElectricIndicatorFailure());
    }
  } catch (err) {
    
    
    yield put(actions.createElectricIndicatorFailure(err));
  }
}

// Individual exports for testing
export default function* addElectricIndicatorPageSaga() {
  yield takeLatest(constants.CREATE_WATERINDICATOR, createElectricIndicator);
}
