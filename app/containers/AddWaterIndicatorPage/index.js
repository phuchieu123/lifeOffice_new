...
import { clean, createWaterIndicator } from './actions';
import reducer from './reducer';
import saga from './saga';
import makeSelectAddWaterIndicatorPage from './selectors';
export function AddWaterIndicatorPage(props) {
  useInjectReducer({ key: 'addWaterIndicatorPage', reducer });
  useInjectSaga({ key: 'addWaterIndicatorPage', saga });

  const { route, navigation, addWaterIndicatorPage, onCreateWaterIndicator, onClean } = props;

  const { isLoading, createWaterIndicatorSuccess } = addWaterIndicatorPage;

  const [avatar, setAvatar] = useState('');
  const [isWaterIndicator, setIsWaterIndicator] = useState(true);
  ...
  useEffect(() => {
    if (createWaterIndicatorSuccess) {
      ToastCustom({ text: 'Thêm mới thành công', type: 'success' });
      handleGoBack();
    } else if (createWaterIndicatorSuccess === false) {
      ToastCustom({ text: 'Thêm mới thất bại', type: 'danger' });
    }
  }, [createWaterIndicatorSuccess]);
  ...
  const handleSubmit = () => {
    ...
    //     taskStage: 1,
    //   };
    //   if (!isWaterIndicator && hasParrent) {
    //     const { task: parent } = route.params;
    //     task.parentId = parent._id;
    //   }
    //   onCreateWaterIndicator({ task });
    //   return;
    // }
...
const mapStateToProps = createStructuredSelector({
  addWaterIndicatorPage: makeSelectAddWaterIndicatorPage(),
});

function mapDispatchToProps(dispatch) {
  return {
    onCreateWaterIndicator: waterIndicator => dispatch(createWaterIndicator(waterIndicator)),
