...
export function ApprovePage(props) {
  ...
  useEffect(() => {
    const approveSuccessEvent = DeviceEventEmitter.addListener(
      'approveSuccess',
      ({approveStatus}) => {
    ...
    const approveSuccessEventCout = DeviceEventEmitter.addListener(
      'ApproveProject',
      ({res}) => {
  ...
  return (
    <View style={{flex: 1}}>
          ...
          options={{
            tabBarLabel: ({focused}) => (
              <View>
                <Text
                  numberOfLines={1}
                  style={{fontSize: 14, color: focused ? 'white' : '#eee'}}>
                  Chờ phê duyệt
                </Text>
                <BadgeView
                  type={0}
                  countApprove={countApprove && countApprove.countNotApproved}
                />
              </View>
            ),
          }}></Tab.Screen>
          ...
          component={() => {
            return (
              <ListPage
                ...
                api={API_APPROVE}
                itemComponent={({item}) => (
                  <ApproveItem item={item} handleOpenModal={handleOpenModal} />
                )}
              />
            );
          }}
          options={{
            tabBarLabel: ({focused}) => (
              <View>
                ...
                <BadgeView
                  type={1}
                  countApprove={countApprove && countApprove.countApproved}
                />
              </View>
            ),
          }}
      ...
      <FabLayout
        onPress={() => navigation.navigate('AddApprovePage')}
        style={styles}>
