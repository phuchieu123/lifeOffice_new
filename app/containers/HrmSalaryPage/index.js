    ...
    <View style={{flex: 1}}>
        ...
        itemComponent={({item}) => {
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() =>
                navigation.navigate('HrmSalaryPageDetail', {item: item})
              }>
              <View>
                <Text>{`Bảng lương tháng ${month} năm ${year}`}</Text>
                {inCharge && (
                  <Text note numberOfLines={1}>{`Phụ trách: ${inCharge}`}</Text>
                )}
                {department && (
                  <Text
                    note
                    numberOfLines={1}>{`Phòng ban: ${department}`}</Text>
                )}
              </View>
              <View>
      ...
      <FabLayout style={styles}>
        <Icon
          ...
          name="plus"
          style={{color: '#fff'}}
          onPress={() => navigate('HrmNewSalaryPage')}
        />
      </FabLayout>
    </View>
  );
