...
export function MeetingScheduleDatailPage(props) {
  ...
  const checkValid = () => {
    let updateList =
      'name, address,code,content,customers,organizer,people,prepare,prepareMeeting,roomMetting,task,timeStart,timeEnd '
        .replace(/ /g, '')
        .split(',')
        .reduce((a, v) => ({...a, [v]: v}), {});
    const {isValid, errorList, firstMessage} = checkedRequireForm(
      calendarConfig,
      localData,
      updateList,
    );
    ...
    if (!err.timeStart && !err.timeEnd) {
      const isValidDate = moment(localData.timeStart).isBefore(
        localData.timeEnd,
      );
      if (!isValidDate) {
                ...
                <ScrollView>
                  <View
                    ...
                    <View style={{flex: 1, backgroundColor: '#fff'}}>
                      {!_.get(calendarConfig, 'name.checkedShowForm') ? null : (
                        <View
                          inlineLabel
                          style={styles.view}
                          error={error.name}>
                          <Text style={{flex: 1}}>
                            {convert(_.get(calendarConfig, 'name.title')) ||
                              'Tên cuộc họp'}
                            :
                          </Text>
                          <TextInput
                      ...
                      {!_.get(calendarConfig, 'code.checkedShowForm') ? null : (
                        <View
                          inlineLabel
                          style={styles.view}
                          error={error.code}>
                          <Text style={{flex: 1}}>
                            {convert(_.get(calendarConfig, 'code.title')) ||
                              'Mã cuộc họp'}
                            :
                          </Text>
                          <TextInput
                      ...
                      {!_.get(
                        ...
                        <View
                          ...
                          <Text>
                            {convert(
                              _.get(calendarConfig, 'timeStart.title'),
                            ) || 'Thời gian bắt đầu'}
                            :
                          </Text>
                          <DateTimePicker
                            ...
                            value={
                              localData.timeStart &&
                              moment(localData.timeStart).format(
                      ...
                      {!_.get(
                        ...
                        <View
                          ...
                          error={error.timeEnd}>
                          <Text>
                            {convert(_.get(calendarConfig, 'timeEnd.title')) ||
                              'Thời gian kết thúc'}
                            :
                          </Text>
                          <DateTimePicker
                            ...
                            value={
                              localData.timeEnd &&
                              moment(localData.timeEnd).format(
                                DATE_FORMAT.DATE_TIME,
                              )
                            }
                          />
                      ...
                      {!_.get(
                        calendarConfig,
                        'roomMetting.checkedShowForm',
                      ) ? null : (
                        <View
                          inlineLabel
                          style={styles.view}
                          error={error.roomMetting}>
                          <Text>
                            {convert(
                              _.get(calendarConfig, 'roomMetting.title'),
                            ) || 'Phòng họp'}
                            :
                          </Text>
                      ...
                      {!_.get(
                        ...
                        <View
                          ...
                          error={error.address}>
                          <Text>
                            {convert(_.get(calendarConfig, 'address.title')) ||
                              'Địa điểm họp'}
                            :
                          </Text>
                      ...
                      {!_.get(calendarConfig, 'task.checkedShowForm') ? null : (
                        <View
                          inlineLabel
                          style={styles.view}
                          error={error.task}>
                          <Text>
                            {convert(_.get(calendarConfig, 'task.title')) ||
                              'Công việc dự án'}
                            :
                          </Text>
                          <SingleAPISearch
                            ...
                            selectedDatas={
                              _.get(meeting, 'task') && [_.get(meeting, 'task')]
                            }
                      ...
                      {!_.get(
                        ...
                        <View
                          ...
                          error={error.prepare}>
                          <Text>
                            {convert(_.get(calendarConfig, 'prepare.title')) ||
                              'Người chuẩn bị'}
                            :
                          </Text>
                          <MultiAPISearch
                            disabled={!PUT}
                            API={API_USERS}
                            onSelectedItemObjectsChange={e =>
                              handleChange('prepare', e)
                            }
                            selectedItems={
                              Array.isArray(_.get(localData, 'prepare')) &&
                              localData.prepare.map(e => e._id)
                            }
                      ...
                      {!_.get(
                        calendarConfig,
                        'prepareMeeting.checkedShowForm',
                      ) ? null : (
                        <View
                          inlineLabel
                          style={styles.view}
                          error={error.prepareMeeting}>
                          <Text>
                            {convert(
                              _.get(calendarConfig, 'prepareMeeting.title'),
                            ) || 'Chuẩn bị cuộc họp'}
                            :
                          </Text>
                          <TextInput
                            multiline
                            disabled={!PUT}
                            value={localData.prepareMeeting}
                            onChangeText={e =>
                              handleChange('prepareMeeting', e)
                            }
                            style={styles.input}
                          />
                          <Icon
                      ...
                      {!_.get(
                        ...
                        <View
                          ...
                          <Text>
                            {convert(
                              _.get(calendarConfig, 'customers.title'),
                            ) || 'Khách hàng'}
                            :
                          </Text>
                          ...
                          <MultiAPISearch
                            ...
                            API={API_CUSTOMER}
                            onSelectedItemObjectsChange={e =>
                              handleChange('customers', e)
                            }
                            selectedItems={
                              Array.isArray(_.get(localData, 'customers')) &&
                              localData.customers.map(e => e._id)
                            }
                      ...
                      {!_.get(
                        ...
                        <View
                          ...
                          error={error.people}>
                          <Text>
                            {convert(_.get(calendarConfig, 'people.title')) ||
                              'Người tham gia'}
                            :
                          </Text>
                          ...
                          <MultiAPISearch
                            ...
                            API={API_USERS}
                            onSelectedItemObjectsChange={e =>
                              handleChange('people', e)
                            }
                            selectedItems={
                              Array.isArray(_.get(localData, 'people')) &&
                              localData.people.map(e => e._id)
                            }
                      ...
                      {!_.get(
                        _.get(calendarConfig, 'organizer.name'),
                        'checkedShowForm',
                      ) ? null : (
                        <View
                          inlineLabel
                          ...
                          <Text>
                            {convert(
                              _.get(calendarConfig, 'organizer.name').title,
                            ) || 'Người tổ chức'}
                            :
                          </Text>
                          <SingleAPISearch
            ...
            return (
              <View style={{flex: 1}}>
                ...
                <FabLayout
                  ...
                  onPress={() => (isLoading ? '' : handleAddFile())}>
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    updateSchedule: data =>
      dispatch(actions.updateScheduleMettingSchedule(data)),
