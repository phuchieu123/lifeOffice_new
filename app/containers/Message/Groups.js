...
const Groups = props => {
  const [query, setQuery] = useState({
    sort: '-updatedAt',
    filter: {type: 1, $expr: {$gt: ['$updatedAt', '$createdAt']}},
  });
  ...
  const formatData = props => {
    ...
    if (
      allConversation &&
      Array.isArray(allConversation.data) &&
      Array.isArray(allNewestMessage) &&
      ...
      result = allConversation.data.map(con => {
        ...
        return {
          ...con,
          message: {
            ...(allNewestMessage.find(
              mes => mes && mes.conversation === con._id,
            ) || {}),
          },
...
const RenderItem = ({item}) => {
  ...
  return (
    <View onPress={() => navigate('MessageChat', {conversation: item})}>
