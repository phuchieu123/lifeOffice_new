...
export function NotificationPage(props) {
  ...
  const reloadCount = () => {
    if (socketError) {
      getNotifications({filter: {isRead: false}, limit: 1, skip: 0}).then(
        e => e && onSocketEvent('notification', {isNotRead: e.count}),
      );
    } else {
  ...
  const handleOpenFilterModal = () => {
    setIsRead(
      _.has(query, 'filter.isRead')
        ? _.get(query, 'filter.isRead')
          ? 2
          : 1
        : null,
    );
        ...
        footerComponent={
          <TouchableOpacity
            ...
            style={{
              ...
              paddingVertical: 10,
              borderWidth: 1,
              borderColor: 'rgba(46, 149, 46, 1)',
              borderRadius: 2
            }}>
            {notificationPage.loading && <ActivityIndicator color="gray" />}
            {!notificationPage.loading && (
              <AntDesign color='rgba(46, 149, 46, 1)' type="AntDesign" name="eye" />
            )}
            {!notificationPage.loading && <Text style={{color:'rgba(46, 149, 46, 1)',paddingLeft:10 }}>Đánh dấu đã đọc</Text>}
          </TouchableOpacity>
        }
      />
      <Modal isVisible={openFilterModal} style={{ height: 'auto' }}>
        <View style={{ backgroundColor: '#fff', borderRadius: 10 }}>
          ...
          <View padder style={{ flexDirection: 'row', marginTop: 40 }}>
            <TouchableOpacity block onPress={handleSaveFilter} style={{ flex: 1, borderRadius: 10, marginRight: 5 }}>
              <IconFeather name="check" type="Feather" />
            </TouchableOpacity>
            <TouchableOpacity block onPress={() => setOpenFilterModal(false)} full style={{ flex: 1, borderRadius: 10, marginRight: 5 }} warning>
              <AntDesign name="close" type="AntDesign" />
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
...
function mapDispatchToProps(dispatch) {
  return {
    onGetNotifications: (query, isLoadMore) =>
      dispatch(actions.getNotifications(query, isLoadMore)),
  ...
  return (
    <View
      ...
      style={{
        marginLeft: 0,
        paddingLeft: 15,
        backgroundColor: isRead ? '#fff' : '#ccc',
      }}
      onPress={onClick}>
      <View>
        <Image source={getAvatar(avatarProfile.avatar, profile.gender)} />
      </View>
      <View>
