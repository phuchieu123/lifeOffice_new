...
export function AddPersonnel(props) {
    ...
    const checkValid = () => {
        let updateList = 'code, name, phoneNumber,email,address,birthday,note,identityCardNumber'
        ...
        if (!localData.identityCardNumber || !localData.identityCardNumber.length) {
            valid = valid && localData.identityCardNumber;
            ToastCustom({ text: 'Nhập trường số CMTND', type: 'danger' })
            err.identityCardNumber = true
        }
        ...
        <>
            ...
            <Content padder style={{ backgroundColor: '#fff' }}>
                <LoadingLayout isLoading={loading}>
                    ...
                    {!_.get(hrmRoles, 'name.checkedShowForm') ? null : <Item inlineLabel style={styles.item} error={error.name}>
                        <Label >{convert(_.get(hrmRoles, 'name.title') || 'Tên nhân sự')}:</Label>
                        <Input multiline={false} style={{ textAlign: 'right', marginRight: 5, paddingTop: 10 }} onChangeText={e => handleChange('name', e)} value={localData.name} placeholder="Tối thiểu 5 kí tự" placeholderTextColor="#ccc" />
                        <Icon active type="Entypo" name="keyboard" style={{ fontSize: 16, color: '#ccc' }} />
                    </Item>}

                    {!_.get(hrmRoles, 'phoneNumber.checkedShowForm') ? null : <Item inlineLabel style={styles.item} error={error.phoneNumber}>
                        <Label >{convert(_.get(hrmRoles, 'phoneNumber.title') || 'Số điện thoại')}:</Label>
                        <Input multiline={false} style={{ textAlign: 'right', marginRight: 5 }} onChangeText={e => handleChange('phoneNumber', e)} maxLength={13} value={localData.phoneNumber} keyboardType="decimal-pad" />
                    ...
                    {!_.get(hrmRoles, 'identityCardNumber.checkedShowForm') ? null : <Item style={styles.item} error={error.identityCardNumber}>
                        ...
                        <Input multiline={false} keyboardType="decimal-pad" maxLength={13} style={{ textAlign: 'right', marginRight: 5 }} onChangeText={e => handleChange('identityCardNumber', e)} value={localData.identityCardNumber} />
                        <Icon active type="Entypo" name="keyboard" style={{ fontSize: 16, color: '#ccc' }} />
                    ...
                    {!_.get(hrmRoles, 'email.checkedShowForm') ? null : <Item inlineLabel style={styles.item} error={error.email}>
                        ...
                        <Input multiline={false} style={{ textAlign: 'right', marginRight: 5 }} onChangeText={e => handleChange('email', e)} value={localData.email} />
                        <Icon active type="Entypo" name="keyboard" style={{ fontSize: 16, color: '#ccc' }} />
                    </Item>}
                    ...
                    {!_.get(hrmRoles, 'address.checkedShowForm') ? null : <Item inlineLabel style={styles.item} error={error.address}>
                        ...
                        <Input multiline={false} style={{ textAlign: 'right', marginRight: 5, paddingTop: 10 }} onChangeText={e => handleChange('address', e)} value={localData.address} />
                        <Icon active type="Entypo" name="keyboard" style={{ fontSize: 16, color: '#ccc' }} />
                    </Item>}
...
export default compose(withConnect, memo)(AddPersonnel);
