...
import { ActionSheet, Button, CardItem, Content, Icon, Input, Item, Label, Right, Text, Textarea, View } from 'native-base';
...
export function DetailsPersonnel(props) {
    ...
    return (
        <>
            <BackHeader
                title={id ? 'Chi tiết nhân sự' : 'Thêm mới nhân sự'}
            ...
            <Content padder style={{ backgroundColor: '#fff' }}>
                <LoadingLayout isLoading={loading}>
                    <AvatarInput source={getAvatar(ava, localData.gender)} onSave={setAvatar} view={!PUT}/>
                    {/* <CustomThumbnail image={localData.avatar ? { uri: localData.avatar } : localData.userImage} style={{
                        flex: 1,
                        margin: 20,
                        width: 120,
                        height: 120,
                    ...
                    {!_.get(hrmRoles, 'code.checkedShowForm') ? null : <Item inlineLabel error={error.code} style={styles.item} disabled>
                        ...
                        <Input multiline={false} style={{ textAlign: 'right', marginRight: 5 }} onChangeText={e => handleChange('code', e)} value={localData.code} disabled={true} />
                    ...
                    {!_.get(hrmRoles, 'name.checkedShowForm') ? null : <Item inlineLabel style={styles.item} error={checkis}>
                        ...
                        <Input multiline={false} style={{ textAlign: 'right', marginRight: 5, paddingTop: 10 }} onChangeText={e => handleChange('name', e)} value={localData.name} placeholder="Tối thiểu 5 kí tự" placeholderTextColor="#ccc" disabled={!PUT}/>
                        <Icon active type="Entypo" name="keyboard" style={{ fontSize: 16, color: '#ccc' }} />
                    </Item>}
                    ...
                    {!_.get(hrmRoles, 'birthday.checkedShowForm') ? null : <Item inlineLabel>
                        ...
                        <DateTimePicker
                            ...
                            value={localData.birthday && moment(localData.birthday).format('DD/MM/YYYY')}
                            disabled={!PUT}
                        />
                    </Item>}
                    ...
                    {!_.get(hrmRoles, 'phoneNumber.checkedShowForm') ? null : <Item inlineLabel style={styles.item}>
                        ...
                        <Input multiline={false} style={{ textAlign: 'right', marginRight: 5 }} onChangeText={e => handleChange('phoneNumber', e)} maxLength={13} value={localData.phoneNumber} keyboardType="decimal-pad" disabled={!PUT}/>
                    ...
                    {!_.get(hrmRoles, 'identityCardNumber.checkedShowForm') ? null : <Item style={styles.item} error={error.identityCardNumber}>
                        ...
                        <Input multiline={false} keyboardType="decimal-pad" maxLength={13} style={{ textAlign: 'right', marginRight: 5 }} onChangeText={e => handleChange('identityCardNumber', e)} value={localData.identityCardNumber} disabled={!PUT}/>
                        <Icon active type="Entypo" name="keyboard" style={{ fontSize: 16, color: '#ccc' }} />

                    </Item>}                

                    {!_.get(hrmRoles, 'email.checkedShowForm') ? null : <Item inlineLabel style={styles.item} error={error.idetityCardNumber}>
                        <Label >{convert(_.get(hrmRoles, 'email.title') || 'Email')}:</Label>
                        <Input multiline={false} style={{ textAlign: 'right', marginRight: 5 }} onChangeText={e => handleChange('email', e)} value={localData.email} disabled={!PUT}/>
                        <Icon active type="Entypo" name="keyboard" style={{ fontSize: 16, color: '#ccc' }} />
                    </Item>}
                    ...
                    {!_.get(hrmRoles, 'address.checkedShowForm') ? null : <Item inlineLabel style={styles.item} error={error.address}>
                        ...
                        <Input multiline={false} style={{ textAlign: 'right', marginRight: 5, paddingTop: 10 }} onChangeText={e => handleChange('address', e)} value={localData.address} disabled={!PUT}/>
                        <Icon active type="Entypo" name="keyboard" style={{ fontSize: 16, color: '#ccc' }} />
                    </Item>}
            ...
            {/* <Button block onPress={() => navigate('ListFilePersonnel', { id: localData._id })} style={{ marginTop: 18, borderRadius: 40 }}>
                    <Text>Danh sách tập tin</Text>
                </Button> */}
            {/* <Button block onPress={() => pickupImage(setDataFace)} style={{ marginTop: 18, borderRadius: 40 }}>
                    <Text>Cài đặt ảnh đại diện khuôn mặt</Text>
                </Button> */}
            {/* </View> */}
...
export default compose(withConnect, memo)(DetailsPersonnel);
