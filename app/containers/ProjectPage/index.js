...
export function ProjectPage(props) {
  ...
  useEffect(() => {
    const updateEvent = DeviceEventEmitter.addListener(
  ...
  const handleFilter = filter => {
    const {
      organizationUnitId: selectedOrg,
    ...
    if ((startDate || endDate) && !newQuery.filter.startDate)
      newQuery.filter.startDate = {};
    if (startDate)
      newQuery.filter.startDate.$gte = `${moment(startDate, DATE_FORMAT)
        .startOf('day')
        .toISOString()}`;
    if (endDate)
      newQuery.filter.startDate.$lte = `${moment(endDate, DATE_FORMAT)
        .endOf('day')
        .toISOString()}`;
  ...
  const customData = ({data, loadMore}) => {
    ...
    newData = newData.map(project => ({
      ...project,
      totalChild: data.filter(e => e.parentId === project._id).length,
    }));
    return newData;
  };
    ...
    <View style={{position: 'relative', flex: 1}}>
      ...
      <CustomHeader
        ...
        rightHeader={
          <RightHeader
            ...
            enableDatePicker
            startDate={
              _.get(query, 'filter.startDate.$gte') &&
              moment(query.filter.startDate.$gte).format(DATE_FORMAT)
            }
            endDate={
              _.get(query, 'filter.startDate.$lte') &&
              moment(query.filter.startDate.$lte).format(DATE_FORMAT)
            }
            enableTaskConfig
      ...
      {!taskRole.POST ? null : (
        <FabLayout
          ...
          children={<Icon type="Entypo" name="plus" style={{color: '#fff'}} />}
        />
      )}
    </View>
  );
