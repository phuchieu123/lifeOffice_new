...
export function* addSalesQuotationsMetting(action) {
  let { data } = action;
  ...
  try {
    ...
    if (response) {
      ...
      yield put(actions.addSalesQuotationsMettingSuccess());
    } else {
      ToastCustom({ text: 'Thêm Lịch Họp thất bại', type: 'danger' })
      yield put(actions.addSalesQuotationsMettingFailure());
    }
  } catch (err) {
    ToastCustom({ text: 'Thêm Lịch Họp thất bại', type: 'danger' })
    yield put(actions.addSalesQuotationsMettingFailure());
...
export function* updateSalesQuotationsMetting(action) {
  let { data } = action;
  ...
  try {
    ...
    if (response) {
      ...
      yield put(actions.updateSalesQuotationsMettingSalesQuotationsSuccess());
    } else {
      ToastCustom({ text: (response && response.message) ? response.message : 'Cập nhật thất bại', type: 'danger' })
      yield put(actions.updateSalesQuotationsMettingSalesQuotationsFailure());
    }
  } catch (err) {
...
export default function* meetingSalesQuotationsPageSaga() {
  // See example in containers/HomePage/saga.js
  yield takeLatest(GET_MEETING_SCHEDULE, fetchMeeting);
  yield takeLatest(ADD_SCHEDULE, addSalesQuotationsMetting);
  yield takeLatest(UPDATE_SCHEDULE, updateSalesQuotationsMetting);
}
